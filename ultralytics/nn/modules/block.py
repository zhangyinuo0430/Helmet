...
from .conv import Conv, CoordAtt, DWConv, GhostConv, LightConv, OcclusionSpatialAttention, RepConv, autopad
...
class C2fCA(C2f):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2f with coordinate attention after feature fusion."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.attn = CoordAtt(c2, c2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2f and coordinate attention."""
        return self.attn(super().forward(x))


class C2fOSA(C2f):
    """C2f block enhanced with occlusion-aware spatial attention."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2f with local-context spatial attention after feature fusion."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.attn = OcclusionSpatialAttention(c2, c2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2f and occlusion-aware attention."""
        return self.attn(super().forward(x))
